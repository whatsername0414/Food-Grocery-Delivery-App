...
import com.vroomvroom.android.view.adapter.ProductsSectionAdapter
...
class MerchantFragment : Fragment(), OnProductClickListener {
    ...
    private var merchant: MerchantQuery.GetMerchant? = null
        ...
        viewModel.merchant.observe(viewLifecycleOwner) { response ->
            when(response) {
                ...
                is ViewState.Success -> {
                    merchant = response.result.getMerchant
                    merchant?.let {
                        val merchantEntity = merchantMapper.mapToDomainModel(it)
                        viewModel.currentMerchant["merchant"] = merchantEntity
    ...
    private fun observeRoomCartItemLiveData() {
        viewModel.cartItem.observe(viewLifecycleOwner, { items ->
            if (items.isNotEmpty()) {
                ...
                binding.cartFabDetail.text = getString(R.string.cart_fab_detail, items.size, subTotal)
                viewModel.isCartCardViewVisible.postValue(true)
